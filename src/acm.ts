...
export function handleACM(message: Message, args: string[], settings: ClientSettings): void {
    if (args.length < 1) {
    ...
    switch (args[0].toLowerCase()) {
        case `info`:
            const acmGeneralChannel = message.guild.channels.get(server.acmGeneralChannel);
